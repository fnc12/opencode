...
/**
 * Drives the permission dock on the emulator (#27), mirroring iOS
 * `PermissionDockUITests`: open a session with an injected pending permission
 * (launch flag `UITEST_PERMISSION`), assert the dock renders, tap **Allow**, and
 * assert it dismisses. Verifies the dock UI + reply wiring without the server
 * being configured to "ask".
 *
...
class PermissionDockUITest {
    ...
    fun approvePermissionFromDock() {
        ...
        ActivityScenario.launch<MainActivity>(intent).use {
            ...
            // The injected permission renders a dock with an Allow button and summary.
            composeRule.waitUntil(timeoutMillis = 15_000) {
                composeRule.onAllNodes(hasTestTag("permission.allow")).fetchSemanticsNodes().isNotEmpty()
            }
            composeRule.onNodeWithText("Run command: echo hello").assertIsDisplayed()

            // Tapping Allow optimistically dismisses the dock.
            composeRule.onNodeWithTag("permission.allow").performClick()
            composeRule.waitUntil(timeoutMillis = 8_000) {
                composeRule.onAllNodes(hasTestTag("permission.allow")).fetchSemanticsNodes().isEmpty()
            }
            assertTrue(
                "dock didn't dismiss after Allow",
                composeRule.onAllNodes(hasTestTag("permission.allow")).fetchSemanticsNodes().isEmpty(),
            )
        }
    }
