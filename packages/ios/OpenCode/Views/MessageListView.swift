...
struct MessageListView: UIViewRepresentable {
    ...
    final class Coordinator: NSObject, UITableViewDelegate {
        ...
        func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
            ...
            // The question-dock footer eases in via its own SwiftUI onAppear (the
            // UIKit willDisplay path here is unreliable for it — the pin-to-bottom
            // re-layout on appearance snaps the transform).
        ...
        func apply(_ messages: [MessageWithParts], table: UITableView, suppressEntrance: Bool = false) {
            ...
            for message in messages {
                let signature = MessageRenderer.signature(message)
                if rendered[message.id]?.signature != signature {
                    rendered[message.id] = (signature, MessageRenderer.render(message))
